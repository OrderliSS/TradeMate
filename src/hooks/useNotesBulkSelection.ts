...
export const useNotesBulkSelection = () => {
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [isCheckboxModeEnabled, setIsCheckboxModeEnabled] = useState<boolean>(() => {
    const saved = localStorage.getItem('notesCheckboxModeEnabled');
    return saved !== null ? saved === 'true' : false;
  });

  useEffect(() => {
    localStorage.setItem('notesCheckboxModeEnabled', String(isCheckboxModeEnabled));
